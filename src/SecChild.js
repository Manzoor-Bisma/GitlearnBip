...
export default function SecChild(props) {
    const {appStyle} = useContext(GlobalInfo);
    const {bStyle} = useContext(GlobalInfo);
    const {fFamily} = useContext(GlobalInfo);
      ...
      return(
       ...
       <div className="card" style={{width: "18rem;"}}>
          ...
          <div className="card-body">
              <hr />  
              <h3 className="card-title text-success fs-1" style={{textAlign:"center" , textDecoration:appStyle}}>{props.Card_title}</h3>
              <h5 className="card-text text-danger fs-3" style={{textAlign:"center" , borderStyle:bStyle}}>{props.card_text}</h5>
          </div>
          <ul className="list-group list-group-flush">
             <li className="list-group-item bg-secondary text-white fs-1" style={{textAlign:"center" , fontFamily:fFamily}}>{props.sec_item}</li>
             <li className="list-group-item bg-warning text-dark fs-1" style={{textAlign:"center" , textShadow:tShadow}} >{props.thr_item}</li>
